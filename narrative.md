...
If you look at the script, you'll see references to environment variables like `$REQUEST_METHOD`. That's how Apache makes information about the request avaliable to the script (as part of the CGI standard). In case you want to see all of them, I've added an [env resource](env/index.cgi), which dumps them all out, plus the content. You can hit it with Chrome or curl, or even netcat. See what's different between them.
...
If I go back and hit the `env` script, I see `HTTP_ACCEPT_LANGUAGE` is now `de,en-US;q=0.8,en;q=0.6`.

## Caching

This is one that I recently avoided re-implementing. We were developing an iPhone app that used a data file that wouldn't change often, but would need to be kept up to date. It was a couple megabytes - more than we'd want to have to fetch every time the app starts up. We were talking about all sorts of ways of doing that before I thought, "Hey, I bet there's some sort of cacheing mechanism built into HTTP." It turns out that's an understatement - there's all kinds of caching schemes built into HTTP.

The simplest and most generally useful involves the `ETag` header. This is a unique identifier generated by the server and included in the response headers. Apache does it automatically for static files. For example, if you fetch this file with `curl`, you'll see a header block like this:
...
Then the next time you request that page, include a `If-None-Match` header with that `ETag` value. That tells the server to only return the contents if the `ETag` value it calculates doesn't match. (Note that the quotes are important!) Otherwise, it returns a 304 status code, like so:
...
* REST API with Bash
    ...
    * Adding other languages with the Accept-language header (load v3)
